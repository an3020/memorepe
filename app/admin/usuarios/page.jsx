...
export default async function AdminUsuarios() {
  ...
  const { data: usuarios } = await admin
    .from('users')
    .select('id, username, email, created_at, plan, role, blocked')
  ...
  for (const s of sesiones || []) {
    sesionesMap[s.user_id] = (sesionesMap[s.user_id] || 0) + 1
  }
  ...
  for (const q of quizzes || []) {
    quizzesMap[q.user_id] = (quizzesMap[q.user_id] || 0) + 1
  }

  const usuariosConStats = (usuarios || []).map(u => ({
    ...u,
    sesiones: sesionesMap[u.id] || 0,
    quizzes: quizzesMap[u.id] || 0,
  }))
    ...
    <div style={{ minHeight: '100vh', background: '#0d0d0d', fontFamily: 'Arial, sans-serif' }}>
      <nav style={{ background: '#111', borderBottom: '1px solid #222', padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
          ...
          <a href="/admin/usuarios" style={{ fontSize: '13px', fontWeight: '500', color: 'white', textDecoration: 'none' }}>Usuarios</a>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 24px' }}>
        <div style={{ marginBottom: '24px' }}>
          <h1 style={{ fontSize: '20px', fontWeight: '500', color: 'white', marginBottom: '4px' }}>Usuarios</h1>
          <p style={{ fontSize: '13px', color: '#6b7280' }}>{usuariosConStats.length} usuarios registrados</p>
        </div>
        <AdminUsuariosClient usuarios={usuariosConStats} />
      </div>
    </div>
  )
}
